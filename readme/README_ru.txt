 ...
 * "HUNSPELL_PREFIX=<path>"
   Задать расположение внешней библиотеки Hunspell. По умолчанию используется
   внутренняя библиотека

 * "ENCA_PREFIX=<path>"
   Задать расположение внешней библиотеки Enca. По умолчанию используется
   внутренняя библиотека

 * "QDOC_COMMAND=<command>"
   Задать команду, используемую для генерирования документации. По умолчанию
   используются команды "qdoc" или "qdoc3"
 ...
 * "BEQT_CONFIG=no_docs"
   Не генерировать документацию. По умолчанию документация генерируется при
   помощи Doxygen. Сгенерированные файлы находятся в папке "doc/doxygen"
...
Важно: Обратите внимание, что подмодуль yourproject зависит от подмодуля beqt.
Таким образом, гарантируется, что BeQt будет собрана перед тем, как подмодуль
yourproject использует ее.
